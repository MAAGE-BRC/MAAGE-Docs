...
server {
	...
	root /MAAGE-Docs/docroot/_build/html;
...
From within the `MAAGE-Docs/docroot` directory you can run this command to rebuild the doc site.
```
make html
```
...
```
singularity run --app build --bind /path/to/clone/of/MAAGE-Docs:/doc_repo /path/to/singularity/image/sphinx.sif
```
...
```
singularity run --app webserver --writable-tmpfs  --bind /path/to/clone/of/MAAGE-Docs:/doc_repo /path/to/singularity/image/sphinx.sif <PORT TO LISTEN ON, Defaults to 8080>
```

## Resources
We write our documentation pages in both Markdown (.md) and reStructuredText (.rst). Sphinx uses the CommonMark version of Markdown when it builds the documentation site. It is slightly different than the GitHub Flavored MarkDown used by GitHub so make sure to take note of any subtle differences. Use these resources to write and develop documentation for MAAGE without causing warnings or failures. There is only one flavor of reStructuredText.
