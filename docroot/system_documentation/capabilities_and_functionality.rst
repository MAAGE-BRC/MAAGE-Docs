...
Primary data types in MAAGE have dedicated data pages that contain dynamically generated content retrieved from the MAAGE databases and queries to other external resources.
...
MAAGE tools and visualizations are interactive components within the website that enable the user to search, retrieve, filter, compare, analyze, graphically portray, and otherwise reformat the presentation of data.  Some tools (e.g., Global Search) have all of the required code in the https://github.com/BV-BRC/bvbrc_website/blob/master/.gitmodules repo, and thus do not have a separate repo 
...
Allows exploration of phylogenetic relationships using species- and genus-level coloring schemes. MAAGE's phylogeny viewer also supports custom creation of genome groups to be used as a basis for analysis in other MAAGE tools.
...
Provides graphical portrayal of the alignment of genes and other genomic data (i.e., genome features) depicted along a central horizontal axis of genome coordinates. MAAGE's genome browser supports comparison of genome annotations from multiple sources (e.g., MAAGE, RefSeq, etc.). Users can upload their own custom tracks. 
