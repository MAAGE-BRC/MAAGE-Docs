...
## Overview
The BLAST service integrates the BLAST (Basic Local Aligment Search Tool) algorithms to perform searches against against public or private genomes in MAAGE or other reference databases using a DNA or protein sequence and find matching genomes, genes, RNAs, or proteins.  
...
The **BLAST** submenu option under the **Services** main menu (Genomics category) opens the BLAST input form (*shown below*). *Note: You must be logged into MAAGE to use this service.*
...
There are three types of Query sources that are provided by MAAGE:
...
MAAGE has different databases to choose from for the source to search wihin: 
...
* **Genome Sequences (NT)** - Genomic sequences from bacterial and viral genomes in MAAGE, i.e. chromosomes, contigs, plasmids, segments, and partial genomic sequences 
* **Genes (NT)** - Gene sequences from bacterial and viral genomes in MAAGE. 
* **Proteins (AA)** - Protein sequences from bacterial and viral genomes in MAAGE.
...
* **blast_headers.txt** - column headers in BLAST results table, which include
  ...
  * evalue: expect value
  * bitscore: bit score
* **blast_out.txt** - Entire file generated by BLAST job. Used by MAAGE to create the blast_out.json file.
* **blast_out.json** -  JavaScript Object Notation (JSON) formatted file, which is a standard data interchange format. It is primarily used for transmitting data between the MAAGE web application and backend servers.
* **blast_out.metadata.json** - JSON-formatted file containing the metadata associated with the BLAST job.
* **blast_out.raw.json** - JSON-formatted file containing list of BLAST identifiers (BLAST uses a different set of identifiers than MAAGE). 
...
* **Pathway:** Loads the Pathway Summary Table containing a list of all the pathways in MAAGE in which the selected features are found.
