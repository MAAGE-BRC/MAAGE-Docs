...
**File Format:**  Currently, you can upload your transcriptomics datasets in
...
### Source ID Type
Dropdown list for specifying the source ID type, e.g., ID, NCBI Gene ID, RefSeq Locus Tag. Used to map genes to genes in the platform, allowing integration of other data into the analysis. Due to differences in annotation that may exist, some genes may go unmapped. Unmapped genes will be excluded from subsequent analysis.
...
### Target Genome
Option for mapping the expression data to a particular genome in the platform.  This will facilitate inclusion of other expression data for that genome in subsequent analysis.
